class Guest < ActiveRecord::Base
  ...
  def can_use_free_package?
    ...
    return true unless previous_session = Guest.find_previous_free_session(device_address)
  ...
  def self.find_by_device_address(addr)
    Guest.order("created_at DESC").find_by(device_address: addr)
  end

  def self.find_previous_free_session(addr)
    Guest.order("created_at DESC").find_by(
      device_address: addr,
